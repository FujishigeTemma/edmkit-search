...
疎結合な設計により、ソフトウェアは以下のような利点を得ます。
- 保守性（変更への強さ）：一部の変更が他の部分に影響を及ぼしにくいため、修正や機能追加などの保守作業が容易になります。疎結合であればシステム成長後の変更も局所的に対応できる一方、密結合だと予期せぬ副作用が生じるリスクを高めます。
- 再利用性：部品同士の依存が小さいほど、特定のコンテキストに縛られない汎用的なコンポーネントが作れます。その結果、あるモジュールやクラスを別のプロジェクトや場面で再利用しやすくなります。疎結合なコンポーネントは単独でも機能しやすいため、流用が簡単です。
- テストのしやすさ：疎結合なコードでは各コンポーネントが独立して動作するため、ユニットテストが書きやすくなります。
- 拡張性：既存のコードに大きな変更を加えずに新しい機能を追加しやすくなります。疎結合な設計では、インタフェースに沿って実装して差し替えるだけで機能拡張が可能です（オープン/クローズドの原則にも適合）。密結合だと新機能追加の際に既存コードの改変が避けられず、バグ混入のリスクが上がります。

結合の種類	概要	結合度
内容結合（Content）	他モジュールの中身を直接編集する	🔴 最悪
共通結合（Common）	グローバル変数を共有する	🔴 高い
外部結合（External）	外部データ形式やAPIに依存する	🟠 やや高い
制御結合（Control）	フラグで相手の動作を制御する	🟠 やや高い
スタンプ結合（Stamp）	必要以上の構造体を渡す	🟡 普通
データ結合（Data）	必要なデータのみを引数で渡す	🟢 望ましい
無結合（Message）	メッセージやイベントだけで通信する（通知型）	🟢 理想的

### 高凝集

凝集度（Cohesion：コヒージョン）は、クラスやモジュール内の機能がどれだけ「関連性の高い目的」にまとまっているかを示す指標です。
- 低凝集（Low Cohesion）：異なる関心事が混在し、保守性や可読性が低下。
- 高凝集（High Cohesion）：1つの責務に集中し、変更しやすく、再利用性が高い。
各関数・クラスは 1つの明確な責務 に集中すべきで、単一責任の原則（SRP：Single Responsibility Principle）に準拠することで、高凝集を目指せます。
高凝集な設計はテストしやすく、修正しやすく、再利用性も高くなります。

結合の種類	概要	結合度
偶然的凝集（Coincidental）	無関係の処理が同じモジュールに集まっている	🔴 最悪
論理的凝集（Logical）	複数の処理を1つの関数で分岐（switch文など）	🟠 低い
時間的凝集（Temporal）	実行タイミングが同じ処理をまとめている（初期化・終了処理など）	🟠 やや低い
手続き的凝集（Procedural）	一連の処理を手順どおりにまとめたが、機能的関連が弱い	🟡 普通
通信的凝集（Communicational）	同じデータにアクセスする処理をまとめている	🟢 やや良い
逐次的凝集（Sequential）	出力を次の処理の入力として渡す一連の処理	🟢 良い
機能的凝集（Functional）	1つの明確な機能に必要な処理のみがまとまっている
...
- `_`プレフィックスで private を表すスタイルは採用しない。
